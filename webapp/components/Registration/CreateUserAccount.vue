  ...
  <div v-else class="create-account-card">
    ...
    <ds-form class="create-user-account" v-model="formData" :schema="formSchema" @submit="submit">
      <template v-slot="{ errors }">
        ...
        <ds-text>
          <input id="checkbox3" type="checkbox" v-model="notCompany" :checked="notCompany" />
          <label
            for="checkbox3"
            v-html="$t('components.registration.signup.form.not-company')"
          ></label>
        </ds-text>
        <ds-button
          style="float: right;"
          ...
          :disabled="
            errors || !termsAndConditionsConfirmed || !dataPrivacy || !minimumAge || !notCompany
          "
          primary
        >
          {{ $t('actions.save') }}
