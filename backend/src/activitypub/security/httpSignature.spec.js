...
describe('activityPub/security', () => {
  ...
  describe('createSignature', () => {
    describe('returned http signature', () => {
      ...
      beforeEach(() => {
        ...
        signer.update(
          '(request-target): post /api/users/max/inbox\ndate: 2019-03-08T14:35:45.759Z\nhost: democracy-app.de\ncontent-type: application/json',
        )
        signatureB64 = signer.sign({ key: privateKey, passphrase }, 'base64')
        httpSignature = createSignature({
          privateKey,
          keyId: 'https://human-connection.org/api/users/lea#main-key',
          url: 'https://democracy-app.de/api/users/max/inbox',
          headers,
          passphrase,
        })
      })
  ...
  describe('verifySignature', () => {
    ...
    beforeEach(() => {
      httpSignature = createSignature({
        privateKey,
        keyId: 'http://localhost:4001/api/users/test-user#main-key',
        url: 'https://democracy-app.de/api/users/max/inbox',
        headers,
        passphrase,
      })
    ...
    it('resolves false', async () => {
      await expect(
        verifySignature('https://democracy-app.de/api/users/max/inbox', headers),
