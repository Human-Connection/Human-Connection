...
;(async function () {
  ...
  try {
    ...
    // https://docs.google.com/spreadsheets/d/1dt2DCnar3JsJtey0K-h6Ls2veog-YOpQTcVEpLrC1tU/edit#gid=0
    // Setup Organization
    const [Org1, Org2] = await Promise.all([
      Factory.build('organization', {
        name: 'Santa Clara County Transit District',
        phone: '408-321-2300',
        webPage: 'https://www.vta.org',
        description: '',
        servicesDesc: '',
      }),
      Factory.build('organization', {
        ...
        phone: '408-294-2100',
        webPage: 'https://www.homefirstscc.org',
        description: '',
        servicesDesc: '',
      }),
    ])
    ...
    const [usa, california, sanjose, santaclara, zip95112] = await Promise.all([
    ...
    const [Org1Address, Org2Address] = await Promise.all([
      Factory.build('location', {
        id: 'address.001',
        name: '2 North First Street',
        lng: -121.8913714,
        lat: 37.3360116,
        type: 'address',
      }),
    ...
    await Promise.all([
      california.relateTo(usa, 'isIn'),
      sanjose.relateTo(california, 'isIn'),
      Org1Address.relateTo(sanjose, 'isIn'),
      Org2Address.relateTo(sanjose, 'isIn'),
      Org1Address.relateTo(zip95112, 'isIn'),
      Org2Address.relateTo(zip95112, 'isIn'),
      Org1Address.relateTo(santaclara, 'isIn'),
      Org2Address.relateTo(santaclara, 'isIn'),
      Org1.relateTo(Org1Address,'isIn'),
      Org2.relateTo(Org2Address,'isIn')
    ])

    // Setup Service
    // Setup Service -> Organization

    // Setup ServiceCategory
    // Setup ServiceCategory -> Service
    // Setup Service -> ServiceCategory

    // Setup Plan

    // Setup Organization -> ServiceCategory
    // Setup Organization -> Service
    // Setup Organization -> Plan
