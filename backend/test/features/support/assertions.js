...
Then('the activity is added to the {string} collection', async function(collectionName) {
  ...
  expect(JSON.parse(response.lastResponse).orderedItems).to.include(
    JSON.parse(this.lastActivity).object,
  )
})

Then('the follower is added to the followers collection of {string}', async function(
  userName,
  follower,
) {
  const response = await this.get(`/api/users/${userName}/followers?page=true`)
  const responseObject = JSON.parse(response.lastResponse)
  expect(responseObject.orderedItems).to.include(follower)
})

Then('the follower is removed from the followers collection of {string}', async function(
  userName,
  follower,
) {
  const response = await this.get(`/api/users/${userName}/followers?page=true`)
...
Then('the object is removed from the outbox collection of {string}', async function(name, object) {
  const response = await this.get(`/api/users/${name}/outbox?page=true`)
...
Then('the post with id {string} has been liked by {string}', async function(id, slug) {
  const shoutedBy = await getPostShoutedBy(id)
